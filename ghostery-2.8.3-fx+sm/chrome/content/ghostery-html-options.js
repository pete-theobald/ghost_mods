...
ghostery.options = {
	...
	CATEGORIES: {
		...
		'tracker': {'full': 'Trackers', 'pop': 'Elements that serve no purpose other than tracking (beacons, conversion pixels, audience segmentation pixels, etc.) fall into the Trackers category'},
	...
	init: function () {
		try {
			...
			if (ghostery.prefs.showSources) { this.GH$('showSources').checked = true; }
	...
	observeForChange: function() {
		this.GH$('panelNew').addEventListener('click', function () { ghostery.options.changes = true; }, true);
		...
		this.GH$('showSources').addEventListener('click', function () { ghostery.options.changes = true; }, true);
	...
	save: function(e) {
		...
		ghostery.prefs.set('showSources', _this.GH$('showSources').checked);
